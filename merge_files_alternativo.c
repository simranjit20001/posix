...
int main(int argc, char *argv[])
{
    ...
    // Allocate the buffer
    /*2 * buffer_size because we need always to have at least one buffer_size of free space
    and we use the read buffer also as the write buffer */
    ...
    int endline, endfile, ret;

    while (files_left || active_buffers > 0)
    {

        while (files[index].fd == -1 && files[index].length == 0)
        {
            index = (index + 1) % N_files;
        }; // Skip the files that are already finished

        endfile = files[index].fd == -1;
        endline = false;
        ret = 0;

        while (files[index].length > 0)
        {
            files[index].length--;
            buffer[offset++] = files[index].buffer[ret + files[index].offset];
            ret++;

            if (files[index].buffer[ret + files[index].offset - 1] == '\n')
            {
                endline = true;
                files[index].offset += ret;
                break;
            }
        }

        if (ret && files[index].length == 0)
        {
            active_buffers--;
        ...
        {
            if (atomicWrite(STDOUT_FILENO, buffer, buffer_size) == ERROR)
            {
                exit(EXIT_FAILURE);
            }
            offset -= buffer_size;
            memcpy(buffer, buffer + buffer_size, offset);
            //We use memcopy instead of memmove because we are sure that the memory areas don't overlap 
        }

        while (!endfile && !endline)
        {

            ret = read(files[index].fd, buffer + offset, buffer_size);
            ...
            {
                endline = true;
                // lseek(files[index].fd, -(ret - i) + 1, SEEK_CUR);
                files[index].length = ret - i - 1;
                if (files[index].length > 0)
                {
                    memcpy(files[index].buffer, buffer + offset,
                           files[index].length);
                    files[index].offset = 0;
                    active_buffers++;
                }
