...
int main(int argc, char *argv[])
{
    ...
    while (files_left)
    {
        ...
        int maxBytes = min(files[index].length - files[index].offset, buffer_size - offset);
        char * ptr = (char *) memchr(
            files[index].buffer + files[index].offset,
            '\n',
            maxBytes
        );

        //calculating the number of bytes we have "read"
        int length = ptr == NULL ? maxBytes : ptr - (files[index].buffer + files[index].offset) + 1; // If ptr is NULL then we have not found a newline
    

        int canChangeBuffers =   
            offset == 0 &&  //main buffer is empty
            files[index].offset == 0 && //file buffer was not used before
            files[index].length == length; //file buffer is now empty (fully copied to main buffer)
        
        if (canChangeBuffers){
            //change buffers to avoid copy
            char * tmp = files[index].buffer;
            files[index].buffer = buffer;
            buffer = tmp;
        } else {
            memcpy(buffer + offset, files[index].buffer + files[index].offset, length);
        }

        files[index].offset += length;
        offset += length;
